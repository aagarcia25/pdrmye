...
const AuthSolicitudes = () => {
  ...
  const [solParticipaciones, setsolParticipaciones] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    permisos.map((item: PERMISO) => {
      if (
        String(item.ControlInterno) === "DPCPAUTHSOL"
      ) {
        //console.log(item);
        if (String(item.Referencia) === "ADMINSOL") {
          setsolParticipaciones(true);
        }
  ...
  return (
    <div>
      ...
      <Grid container spacing={1} padding={2}>
        ...
        <Grid container spacing={1} item xs={12} sm={12} md={12} lg={12}>
          <Grid item xs={4} sm={4} md={3.5} lg={3}>
        ...
        <Grid item xs={12} sm={12} md={12} lg={12}>
          ...
          >
            ...
            <MUIXDataGridMun modulo={""} handleBorrar={handleBorrar} columns={columnsParticipaciones} rows={data} borrar={false} />
